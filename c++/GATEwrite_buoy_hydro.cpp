...
void write_netcdf_buoy_hydro(const std::string &infile,
    ...
    ws_id  = defineVariableAndAttribute<float, 2>(ncid, dimids, "ws",  "wind_speed",          "wind speed",          metadata.wind_unit,        fill_value_999);
    wd_id  = defineVariableAndAttribute<float, 2>(ncid, dimids, "wd",  "wind_from_direction", "wind from direction", metadata.wind_dir_unit,    fill_value_999);
    ...
    psl_id = defineVariableAndAttribute<float, 2>(ncid, dimids, "psl", "sea_level_pressure",  "sea level pressure",  metadata.pressure_unit,    fill_value_999);
    ...
    handle_err(nc_put_att_text(ncid, NC_GLOBAL, "buoy_start_position", position_str.length(), position_str.c_str()));
    ...
    std::vector<float> psl(no_of_measurements);

    for (int i = 0; i < no_of_measurements; ++i) {
      auto& rec = dbuoydata[i];
      auto validw = [](auto w) { return int(w) == 0; };

      measurement_time[i] = rec.seconds_of_year;

      ws[i]  = validw(rec.val_wind_speed)           ? rec.wind_speed                     : fill_value_999;
      wd[i]  = validw(rec.val_wind_direction)       ? rec.wind_direction                 : fill_value_999;
      dbt[i] = validw(rec.val_dry_bulb_temperature) ? rec.dry_bulb_temperature + 273.15f : fill_value_999;
