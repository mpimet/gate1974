...
### Converting GATE Radiossonde profiles from NetCDF to zarr
...
zarr_file="GATE_RADIOSONDES.zarr"
```

```python
def open_dataset(ncfile):
    """Open radiosonde datasets and prepare for concatenation."""
...
```python
root_dir = pathlib.Path(root_path+"/RADIOSONDE")
ncfiles = root_dir.glob("**/*.nc")
```
...
ylim = ax.set_ylim(0, 20_000)
xlim = ax.set_xlim(180, 310)
```
