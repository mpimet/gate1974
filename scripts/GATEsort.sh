...
do
    if test -f ${infile}; then
		...
		echo $TYPE $PLATFORM
		    ...
		    this_dir=BUOY/"${ship}"/"${md5_format_string:0:6}"
		    [[ ! -d "${this_dir}" ]] && mkdir -p "${this_dir}"
		    ...
		    [[ ! -d "${this_dir}" ]] && mkdir -p "${this_dir}"
                ...
                elif [[ "${TYPE}" == "A/C " || "${TYPE}" == " A/C" || "${TYPE}" == "ARCR" ]] ; then
		    ...
		    md5_format_string=`echo ${format_string} | md5sum |tr -d "-"`
                    ...
                    echo ship data from ${PLATFORM}

		    this_dir=AIRCRAFT/"${aircraft}"/"${md5_format_string:0:6}"
		    [[ ! -d "${this_dir}" ]] && mkdir -p "${this_dir}"
