...
subroutine convert_data (infile)
  ...
  metadata%title           = 'NCAR Electra atmospheric in-flight measurements during GATE'
  metadata%summary         = 'In-flight measurements from NCAR Electra collected during ' // &
                             'Global Atmospheric Research Program''s Atlantic Tropical Experiment (GATE, 1974).'
  metadata%chief_scientist = 'Kelley'
...
subroutine write_netcdf ( infile, no_of_measurements, aircraftdata, metadata )
  ...
  std_p_id = define_variable_and_attribute_real( &
       ...
       'standard deviation of air pressure', metadata%pressure_unit, 99.99)

  alt_id = define_variable_and_attribute_real( &
       ncid, dimids, 'z', 'altitude', &
       'altitude', metadata%altitude_unit, 99999.0)

  std_alt_id = define_variable_and_attribute_real( &
       ncid, dimids, 'std_z', 'standard_deviation_of_altitude', &
       'standard deviation of altitude', metadata%altitude_unit, 999.9)
  ...
  std_dew_id = define_variable_and_attribute_real( &
       ...
       'standard deviation of dew point temperature', metadata%temperature_unit, 999.9)
  ...
  call handle_err(nf_put_vara(ncid, alt_id,     start, edge, aircraftdata(1:no_of_measurements)%altitude))
  call handle_err(nf_put_vara(ncid, std_alt_id, start, edge, aircraftdata(1:no_of_measurements)%std_altitude))
