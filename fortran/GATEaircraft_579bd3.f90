...
subroutine convert_data (infile)
  ...
  metadata%chief_scientist = 'Blot, LeJeune, Roumiguieres and Sap'
...
subroutine write_netcdf ( infile, no_of_measurements, aircraftdata, metadata )
  ...
  ta1_id = define_variable_and_attribute_real( &
       ...
       'air temperature', metadata%temperature_unit, 999.0)

  ta2_id = define_variable_and_attribute_real( &
       ncid, dimids, 'ta_2', 'air_temperature', &
       'air temperature', metadata%temperature_unit, 999.0)

  tas_id = define_variable_and_attribute_real( &
       ncid, dimids, 'tas', 'surface_temperature', &
       'surface temperature', metadata%temperature_unit, 999.0)
  ...
  call handle_err(nf_put_vara(ncid, tas_id, start, edge, tas))
