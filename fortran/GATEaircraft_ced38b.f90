...
use GATEaircraft_mod
  ...
  case ( "1" )
     ...
     if ( line(2:68) /= "(I1,I4,I10,I5,12(F6.0,F7.3,F8.3,F6.1,F5.2,F5.0,F5.1,F5.1,F5.2,F5.1," ) then
        write ( * , * )
        write ( * , * ) trim(infile), ':'
...
110 format(I1,I4,I10,I5,12(F6.0,F7.3,F8.3,F6.1,F5.2,F5.0,F5.1,F5.1,F5.2,F5.1,                &
...
subroutine write_netcdf ( infile, no_of_measurements, aircraftdata, metadata )
  ...
  integer :: altitude_id,       std_altitude_id
  ...
  call handle_err(nf_def_var(ncid, "std_z",   NF_FLOAT, ndims, dimids, std_altitude_id))
  ...
  call handle_err(nf_put_att_real(ncid, std_pressure_id, "_FillValue", NF_REAL, 1, 99.99))
  ...
  call handle_err(nf_put_att_real(ncid, altitude_id, "_FillValue", NF_REAL, 1,  99999.))

  call handle_err(nf_put_att_text(ncid, std_altitude_id, "standard_name", 30, "standard_deviation_of_altitude"))
  call handle_err(nf_put_att_text(ncid, std_altitude_id, "units", len(metadata%altitude_unit), metadata%altitude_unit))
  call handle_err(nf_put_att_real(ncid, std_altitude_id, "_FillValue", NF_REAL, 1, 999.9))
  ...
  call handle_err(nf_put_att_text(ncid, dp_temperature_id, "standard_name", 20, "dew_point_temperature"))
  call handle_err(nf_put_att_text(ncid, dp_temperature_id, "long_name", 20, "dew point temperature"))
  ...
  call handle_err(nf_put_att_text(ncid, std_dp_temperature_id, "standard_name", 42, "standard_deviation_of_dew_point_temperature"))
  call handle_err(nf_put_att_text(ncid, std_dp_temperature_id, "long_name", 42, "standard deviation of dew point temperature"))
  ...
  call handle_err(nf_put_vara(ncid, altitude_id,           start, edge, aircraftdata(1:no_of_measurements)%altitude))
  call handle_err(nf_put_vara(ncid, std_altitude_id,       start, edge, aircraftdata(1:no_of_measurements)%std_altitude))
