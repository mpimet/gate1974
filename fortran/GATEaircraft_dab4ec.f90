...
module GATEaircraft_mod
  ...
  integer, parameter :: no_of_records_in_line = 14 ! see format string 110
  ...
  type :: GATE_aircraft_type
     ...
     real :: TEMPERATURE2
     real :: DEW_POINT_TEMPERATURE
     real :: RADIATIVE_TEMPERATURE
...
use GATEaircraft_mod
  ...
  ! ASCII Input
  !
  ! from 3.60.02.105-3.64.02.101_19740601-197409309740930
  ...
  case ( "1" )
     ...
     if ( line(2:61) /= "(I1,I4,I10,I5,14(F7.0,F5.1,F5.1,F7.3,F7.3,F6.1,F6.0,F5.0,F5." ) then
        write ( * , * )
        write ( * , * ) trim(infile), ':'
...
subroutine convert_data (infile)
  ...
  do i = 1, 24
     ...
     case ( 0 )
        ...
        if ( i == 7 ) then
           read(line(17:26),'(F9.1,A1)') interval, metadata%interval_unit
     ...
     if ( ierror == 0 ) then
        ...
        do i = 1, no_of_records_in_line
           ...
           if ( abs(flightdata(i)%latitude  ) <  90.0 .AND. &
                ...
                abs(flightdata(i)%time/1000000.0) > 0.0 .AND. & 
                flightdata(i)%time > 0.0 .AND. flightdata(i)%time < 240000.0 ) then

              ! convert time signal
              call hercules_time_converter(flightdata(i)%time, hour, minute, second)
...
110 format(I1,I4,I10,I5,14(F7.0,F5.1,F5.1,F7.3,F7.3,F6.1,F6.0,F5.0,F5.                  &
                           1,F6.2,F6.2,F5.1,F5.1,F6.1,F6.1,F5.1,F6.1,F6.1,F5.1,F5.1,F5. &
                           1,F5.1,F5.1,F5.1), 24X)
...
subroutine write_netcdf ( infile, no_of_measurements, aircraftdata, metadata )
  ...
  write (history, "(A,I4,A1,I0.2,A1,I0.2,A1,3(I0.2,A1),A)" ) &
                      ...
                      "from data in archive directory 3.60.02.105-3.64.02.101_19740601-197409309740930."
  ...
  call handle_err(nf_put_att_text(ncid, specific_humidity_id, "standard_name", 17, "specific_humidity"))
  ...
  do i = 1, no_of_measurements
     ...
     if ( aircraftdata(i)%pressure > 1100.0 ) then
        p(i) = 999999.9
     else
        p(i) = aircraftdata(i)%pressure * 100.0
     endif

     z(i) = aircraftdata(i)%radar_height

     if ( aircraftdata(i)%temperature1 > 400.0 ) then
        ta(i) = 999.9
     else
        ta(i) = aircraftdata(i)%temperature1 + 273.15
     endif

     if ( aircraftdata(i)%temperature2 > 400.0 ) then
        ta_2(i) = 999.9
     else
  ...
  call handle_err(nf_put_vara(ncid, dew_point_temperature_id, start, edge, &
                              aircraftdata(1:no_of_measurements)%dew_point_temperature))
