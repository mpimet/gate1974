...
program GATEaircraft
  ...
  case ( "1" )
     ...
     if ( line(2:68) /= "(I1,I4,I10,I5,14(F6.0,F7.3,F8.3,F6.1,F5.2,F5.1,F5.2,F5.1,F5.2,F5.1," ) then
        write ( * , * )
        write ( * , * ) trim(infile), ':'
...
subroutine convert_data (infile)
  ...
  metadata%title           = 'NCAR Queen Air atmospheric in-flight measurements during GATE'
  metadata%summary         = 'In-flight measurements from NCAR Queen Air collected during ' // &
                             'Global Atmospheric Research Program''s Atlantic Tropical Experiment (GATE, 1974).'
  metadata%chief_scientist = 'A. Rodi'
 ...
 ! file section 2, metadata of sampled variables
  ...
  do while ( ierror >= 0 )
     ...
     if ( ierror == 0 .and. ierror /= iostat_end ) then
...
110 format(I1,I4,I10,I5,14(F6.0,F7.3,F8.3,F6.1,F5.2,F5.1,F5.2,F5.1,F5.2,F5.1, &
                           F5.2,F5.1,F5.2,F5.1,F5.2,F5.1,F5.2,F5.1,F5.1,F5.1,F5.1,F5.1,14I1)) 
